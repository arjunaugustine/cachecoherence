...
//This function should tell if the cache block can be supplied by another cache; Note that in case of MSI/MOSI/MOESI, 
//if all the processors are in S state or I state, the cache block has to come from main Memory. Only, M or O act as suppliers of cache blocks.
int c2c_supplier(ulong addr, int proc_no) {
	cache_state state;
	for (int i = 0; i < num_processors; i++){
		if (i!=proc_no) {
			//Implement the function such that if you have a processor that can supply the cache block to another processor, 
			//this function returns a non-zero value otherwise a 0.
			...
			if (processor_cache[i]->find_line(addr) != NULL) {
				state=processor_cache[i]->find_line(addr)->get_state();
				if (state == M || state == O || state == E) 
				{
					return 1;
				}
			}
